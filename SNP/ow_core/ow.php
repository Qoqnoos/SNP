...
final class OW
{
    ...
    private static function detectContext()
    {
        ...
        else if ( UTIL_Browser::isSmartphone() )
        {
            $context = self::CONTEXT_MOBILE;
        }
        else if ( defined('OW_API') )
        {
            $context = self::CONTEXT_API;
        }

        $nContext = OW::getEventManager()->call('base.change_context');

        if ( in_array($nContext, array(self::CONTEXT_API, self::CONTEXT_DESKTOP, self::CONTEXT_MOBILE)) )
        {
            $context = $nContext;
        }
    ...
    {
        self::detectContext();
        //printVar(self::$context);
        switch ( self::$context )
        {
            case self::CONTEXT_DESKTOP:
