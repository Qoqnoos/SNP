...
var OWM_Console = function( params )
{
    ...
    this.showCounter = function(counter, tab){
    ...
    $("body")
        .on("click", "li.owm_sidebar_msg_disabled", function(){
            OW.error(params.disabledMessage);
        });
    ...
    initForm: function( textareaId, submitId ){
        ...
        $textA.unbind('focus').one('focus', taHandler).bind('blur',
            function(){
                if( $(this).val() == '' ){
                    $submitCont.hide();
                    $(this).unbind('focus').one('focus', taHandler).val(self.taMessage).addClass('invitation');
                }
            }
        );

        OW.bind('base.comment_add', function(){$textA.blur();});
