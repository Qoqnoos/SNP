...
class BOL_QuestionDao extends OW_BaseDao
{
    ...
    public function findQuestionsForAccountType( $accountType )
    {
        ...
        $sql = " SELECT `question`.* FROM `" . $this->getTableName() . "` as `question` 
                    ...
                    WHERE ( `question`.`accountTypeName` = :accountTypeName )
                    ORDER BY IF( `section`.`name` IS NULL, 0, 1 ),  `section`.`sortOrder`, `question`.`sortOrder` ";
    ...
    public function findSearchQuestionsForAccountType( $accountType )
    {
        ...
        $sql = " SELECT `question`.* FROM `" . $this->getTableName() . "` as `question`
                    ...
                    WHERE ( `account`.`name` IS NULL OR `question`.`accountTypeName` = :accountTypeName OR :accountTypeName = 'all' ) AND `question`.`onSearch` = 1
                    ORDER BY IF( `section`.`name` IS NULL, 0, 1 ),  `section`.`sortOrder`, `question`.`sortOrder` ";
    ...
    public function findAllQuestionsWithSectionForAccountType( $accountType )
    {
        ...
        $sql = " SELECT `q`.* FROM
                    (
                       SELECT `question`.`id`, `question`.`name`, `section`.`name` as `sectionName`, `question`.`accountTypeName`,
                        ...
                        RIGHT JOIN  `" . BOL_QuestionSectionDao::getInstance()->getTableName() . "` as `section`
                                ON ( `question`.`sectionName` = `section`.`name` AND `question`.`sectionName` != '' AND `question`.`sectionName` IS NOT NULL )

                        LEFT JOIN  " . BOL_QuestionAccountTypeDao::getInstance()->getTableName() . " as `account`  ON( `account`.`name` = `question`.`accountTypeName` )

                        WHERE `question`.`id` IS NULL AND ( `account`.`name` IS NULL OR `question`.`accountTypeName` = :accountTypeName OR :accountTypeName = 'all' )

                    ) as `q`

                    ORDER BY IF( `q`.`sectionName` IS NULL, 0, 1 )  ASC,  `q`.`sectionOrder`, `q`.`sortOrder` ";
    ...
    public function findBaseSignUpQuestions()
    {
        $sql = " SELECT `question`.* FROM `" . $this->getTableName() . "` as `question` 
                    ...
                    WHERE `question`.`base` = 1 AND `question`.`onJoin` = '1'
                    ORDER BY IF( `section`.`name` IS NULL, 0, 1 ),  `section`.`sortOrder`, `question`.`sortOrder` ";
